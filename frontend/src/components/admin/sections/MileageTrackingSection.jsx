...
export default function MileageTrackingSection({ getAuthHeader, onTripStatusChange, forceExpand, headerTripActive, headerTripPaused }) {
  ...
  // ============================================
  // "Forgot to Track" Feature Functions
  // ============================================
  
  // Search for addresses with debouncing
  const searchAddresses = useCallback(async (query, type) => {
    if (query.length < 3) {
    ...
    try {
      const response = await axios.get(`${API}/admin/mileage/geocode`, {
        params: { query },
        ...getAuthHeader()
      });
      setAddressSuggestions(prev => ({ ...prev, [type]: response.data || [] }));
    } catch (error) {
  ...
  const handleAddressInput = useCallback((value, type) => {
    setForgotTripData(prev => ({
      ...prev,
      [type === 'start' ? 'start_address' : 'end_address']: value
    }));
  ...
  const selectAddress = useCallback((suggestion, type) => {
    setForgotTripData(prev => ({
      ...prev,
      [type === 'start' ? 'start_address' : 'end_address']: suggestion.display_name
    }));
    if (type === 'start') {
  ...
  // Calculate route between start and end addresses
  const calculateForgotRoute = useCallback(async () => {
    if (!forgotTripData.start_address || !forgotTripData.end_address) {
    ...
    try {
      const response = await axios.post(
        `${API}/admin/mileage/calculate-route`,
        null,
        {
          params: {
            start_address: forgotTripData.start_address,
            end_address: forgotTripData.end_address
          },
          ...getAuthHeader()
        }
      );
      setForgotTripRoute(response.data);
      toast.success(`Route found: ${response.data.distance_miles.toFixed(2)} miles`);
    } catch (error) {
      console.error("Route calculation error:", error);
      toast.error(error.response?.data?.detail || "Could not calculate route");
      setForgotTripRoute(null);
    } finally {
      setIsCalculatingRoute(false);
    }
  }, [forgotTripData.start_address, forgotTripData.end_address, getAuthHeader]);

  // Save the "forgot to track" trip
  const saveForgotTrip = useCallback(async () => {
    if (!forgotTripRoute) {
    ...
    try {
      const response = await axios.post(
        `${API}/admin/mileage/forgot-to-track`,
        null,
        {
          params: {
            start_address: forgotTripData.start_address,
            end_address: forgotTripData.end_address,
            date: forgotTripData.date,
            purpose: forgotTripData.purpose,
            purpose_other: forgotTripData.purpose_other || null,
            notes: forgotTripData.notes || null
          },
          ...getAuthHeader()
        }
      );
      
      toast.success(`Trip added: ${response.data.total_miles} miles`);
      setShowForgotToTrackModal(false);
      setForgotTripData({
        date: new Date().toISOString().split('T')[0],
        start_address: "",
        end_address: "",
  ...
  }, [forgotTripRoute, forgotTripData, getAuthHeader, fetchMileageEntries, fetchCumulativeDistance]);
