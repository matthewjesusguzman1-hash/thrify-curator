...
export default function FormSubmissionModal({
  ...
  // Send scheduling invite using the new scheduler system
  const handleSendSchedulerInvite = async () => {
    setSendingInvite(true);
    try {
      await axios.post(
        `${API}/api/interview-scheduler/admin/send-invite/${submission.id}`,
        {},
        getAuthHeader()
      );
      toast.success(`Scheduling link sent to ${submission.full_name}!`);
      // Update local state to show invite was sent
      submission.scheduler_invite_sent = true;
      submission.scheduler_invite_sent_at = new Date().toISOString();
      if (refreshData) refreshData();
    } catch (error) {
      console.error("Error sending scheduler invite:", error);
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            if (val === '') {
                              ...
                              const maxItems = submission.items_to_add > 0 ? submission.items_to_add : 9999;
          ...
          {submission.formType === "item_additions" && (
            <div className="space-y-4">
              ...
              {/* Consignment Review Section - Only show for item additions, not info-only updates */}
              {submission.items_to_add > 0 && (
                <div className="mt-6 p-4 bg-gradient-to-r from-[#10B981]/10 to-[#059669]/10 rounded-xl border border-[#10B981]/20">
                  ...
                  {submission.approval_status && submission.approval_status !== 'pending' && submission.approval_status !== 'info_update' ? (
                    /* Show review details if already reviewed */
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-3 bg-white rounded-lg">
                    ...
                    <div className="space-y-4">
                      ...
                      {submission.items_to_add > 0 && approvalForm.items_accepted < submission.items_to_add && (
                      <div className="space-y-2">
                        <Label className="text-sm font-medium">
                          What happens to {submission.items_to_add - approvalForm.items_accepted} item(s) not consigned?
                        </Label>
