...
export const SPLIT_LEARN_SCENARIOS = [
  {
    ...
    pairingNote: "TWO qualifying Period B candidates (2h OFF at 17-19 AND 2h OFF at 22-24) each pair with the 7h SB at 06-13. Either valid pair resolves the 14-hr violation that exists under the straight rule, so the driver is compliant — the inspector doesn't have to pick the 'best' one, just confirm at least one covers it.",
    description:
      "Under the straight 14-hr rule (no split): shift STARTS at 00:00 (first OD) and ENDS at 14:00. Driving 14:00-17:00 is past the 14 → 14-hr violation.\n\nPair A (7h SB + 2h OFF 17-19): current split shift = 13:00 → 17:00. Wall-clock 4h (under 14). Driving 4h (under 11). LEGAL — the 14-hr violation is resolved. Pair completes at 19:00 and a fresh shift begins, so the 3h D at 19-22 sits under its own new 11/14 limits.\n\nPair B (7h SB + 2h OFF 22-24): current split shift = 13:00 → 22:00. Wall-clock 9h (the non-pair 2h OFF at 17-19 still counts toward the 14, but total stays under 14). Driving 4h + 3h = 7h (under 11). LEGAL — the 14-hr violation is also resolved.\n\nBoth pairs are legal and that alone is enough. Pair A happens to be more operationally efficient (resets sooner at 19:00 vs 24:00), but Pair B is equally compliant.",
  },
  {
    id: "SL1",
