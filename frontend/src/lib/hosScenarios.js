...
export const LEARN_CONTENT = {
  duty: {
    ...
    roadside: [
      ...
      "Challenge any OFF block where the driver was clearly still responsible for the CMV or cargo — that's misclassified and should be OD.",
      "Any yard movement on private property is OD, not D. Any movement on a public road is D regardless of distance.",
    ],
  },
  ...
  "14hr": {
    ...
    roadside: [
      ...
      "Any driving or on-duty work past the 14-hour mark is a violation — cite §395.3(a)(2).",
      "OOS only applies if the driver is ACTIVELY DRIVING at the time of the 14-hour violation. Violations on a prior day's log: cite, do NOT place OOS.",
      "A valid split-sleeper pair (§395.1(g)(1)(ii)) changes the shift boundaries — see the Split Sleeper Trainer for how to identify them.",
    ],
  },
  ...
  "11hr": {
    ...
    roadside: [
      ...
      "The moment the cumulative D total exceeds 11 hours, any further driving is a violation — cite §395.3(a)(3)(i).",
      "OOS only applies if the driver is ACTIVELY DRIVING at the time of the 11-hour violation. Violations on a prior day's log: cite, do NOT place OOS.",
    ],
  },
  ...
  recap: {
    ...
    roadside: [
      ...
      "Check for a 34-hr consecutive OFF/SB block since the oldest day — if present, the recap resets to zero and you only count hours after the restart.",
      "Use the 60/7 limit instead of 70/8 when the carrier does NOT operate every day of the week.",
    ],
  },
};
...
export const SPLIT_LEARN_SCENARIOS = [
  {
    id: "SL0",
    title: "Inspector principle · any valid pair defends the driver",
    log: [
      { status: "OD",  start: "00:00", end: "01:00" },
      { status: "D",   start: "01:00", end: "06:00" }, // 5h D — prior shift
      { status: "SB",  start: "06:00", end: "13:00" }, // 7h SB — Period A
      { status: "D",   start: "13:00", end: "17:00" }, // 4h D
      { status: "OFF", start: "17:00", end: "19:00" }, // 2h OFF — Period B candidate #1
      { status: "D",   start: "19:00", end: "22:00" }, // 3h D
      { status: "OFF", start: "22:00", end: "24:00" }, // 2h OFF — Period B candidate #2
    ],
    qualifyingBrackets: [
      { startMin: 6 * 60,  endMin: 13 * 60, label: "7h SB ✓ Period A", color: "#10B981" },
      { startMin: 17 * 60, endMin: 19 * 60, label: "2h OFF ✓ Pair A", color: "#10B981" },
      { startMin: 22 * 60, endMin: 24 * 60, label: "2h OFF ✓ Pair B", color: "#10B981" },
    ],
    countedBrackets: [
      { startMin: 13 * 60, endMin: 17 * 60, label: "Pair A · shift 13-17 · 4h D", color: "#D4AF37" },
    ],
    shiftMarkers: [
      { min: 13 * 60,  kind: "start", label: "Pair A START · 13:00", labelRow: 0 },
      { min: 17 * 60,  kind: "end",   label: "Pair A END · 17:00", labelRow: 1 },
    ],
    pairingNote: "TWO qualifying Period B candidates (2h OFF at 17-19 AND 2h OFF at 22-24) can each pair with the 7h SB at 06-13. Under the straight 14-hr rule, D 14-17 is past the 14-hr work-shift limit → violation. But EITHER valid pair resolves it, so the driver is compliant. The inspector doesn't have to pick the 'best' pair — they just have to confirm at least one valid pair covers the otherwise-violating driving.",
    description:
      "A driver can pair any two qualifying rest periods. This log has TWO qualifying Period B candidates (2h OFF at 17-19 AND 2h OFF at 22-24), either of which pairs with the 7h SB at 06-13 for a valid §395.1(g)(1)(ii) split.\n\nUnder the straight 14-hr rule (no split): shift STARTS at 00:00 (first OD) and ENDS at 14:00 (14h wall-clock). Driving 14:00-17:00 is past the 14 → 14-hr violation.\n\nPair A (7h SB + 2h OFF 17-19): current split shift = 13:00 → 17:00. Wall-clock in window = 4h (under 14). Driving in window = 4h (under 11). LEGAL — the 14-hr violation is resolved. The pair completes at 19:00 and a fresh shift begins, so the 3h D at 19-22 sits under its own new 11/14 limits.\n\nPair B (7h SB + 2h OFF 22-24): current split shift = 13:00 → 22:00. Wall-clock in window = 9h (the non-pair 2h OFF at 17-19 still counts toward the 14, so total is 9h, under 14). Driving in window = 4h + 3h = 7h (under 11). LEGAL — the 14-hr violation is also resolved.\n\nInspector principle: if a violation exists under the straight rule, look for ANY valid split pair that would cover it. If at least one exists, the driver is compliant. Pair A also happens to be more operationally efficient (resets sooner at 19:00 vs. 24:00), but both are legal and that alone is enough.",
  },
  {
    id: "SL1",
    ...
    description: "This log has a valid 7+3 split under §395.1(g)(1)(ii): the 7h SB (06-13, Period A) paired with the 3h OFF (17-20, Period B) totals 10h equivalent reset. CVSA procedure kicks in: the CURRENT shift is 13:00 (end of Period A · 7h SB) → 17:00 (beginning of Period B · 3h OFF) with only 4h D counted. After Period B ends at 20:00, Period A + Period B together satisfy the full 10-hr equivalent reset, so a fresh 14-hr window reopens at 20:00. Other valid splits on this log shape: 7+3, 7.5+2.5, 8+2 — any combination ≥7h SB + ≥2h SB/OFF totaling ≥10h.",
    extraExamples: [
      {
        ...
        description: "Non-standard but legal 7.5+2.5 pairing under §395.1(g)(1)(ii). CVSA split shift 13:30 → 17:30 counts 4h D. Whole-hour segments aren't required — any combination ≥7h SB + ≥2h SB/OFF totaling ≥10h qualifies.",
      },
      {
        ...
        description: "Order of segments doesn't matter under §395.1(g)(1)(ii) — here the SHORT period (3h OFF, 06-09) came FIRST and the 7h SB (14-21) came SECOND. CVSA split shift: 09:00 (end of first segment · 3h OFF) → 14:00 (start of second segment · 7h SB), 5h D counted.",
      },
    ],
  },
  {
    ...
    extraExamples: [
      {
        ...
        description: "The SB period doesn't have to be exactly 8h — a 9+2 pairing qualifies just as well. CVSA split shift 07:00 → 12:00 counts 5h (4h D + 1h OD).",
      },
      {
        ...
        description: "The 8h SB comes first (01-09) and the 2h OFF comes second (18-20). CVSA split shift 09:00 → 18:00 counts 9h (5h D + 1h OD + 3h D) — driving total 8h, within 11.",
      },
    ],
  },
  {
    ...
    extraExamples: [
      {
        ...
        qualifyingBrackets: [],
        countedBrackets: [
          { startMin: 0,       endMin: 6 * 60,  label: "Pre-shift OFF · 6h", color: "#64748B" },
        ...
        description: "Shift starts at 06:00 (first D entry). The SB attempt (11:00-17:30) IS a split try AFTER work began, but 6.5h < 7h threshold so the pairing fails (§395.1(g)(1)(ii)). Without a valid split, the 14-hr wall-clock closes at 20:00. Driving at 23:30 = 3.5h past 14h shift end → 14-hr violation.",
      },
      {
  ...
  },
  {
    ...
    description: "Driver's 14h window runs from 00:00 (first OD entry) to 14:00. Attempts 8h OFF (08-16) + 2h SB (19-21) pairing, but §395.1(g)(1)(ii) requires ≥7 consecutive hours in the Sleeper Berth — Off Duty doesn't substitute, and the 2h SB is too short to be the qualifying SB period on its own. No valid split → no exclusion, 14h wall-clock closes at 14:00. Driving 16:00-19:00 is PAST the 14h shift end → 14-hr violation. Splits that WOULD have worked on this shape: 7+3 (SB 08-15 + OFF 19-22), 8+2 (SB 08-16 + OFF 19-21 — just swap statuses of the 8h block), 10+0 (straight 10h SB).",
    extraExamples: [
      {
        ...
        description: "10 consecutive hours Off Duty IS a valid 10-hr continuous reset (§395.3(a)(1)) — NOT a split-sleeper pairing (there's no SB time). Under CVSA 10-Hour Continuous Break rule: Shift 1 ENDS at 06:00 (beginning of the 10h OFF reset). After the 10h OFF completes at 16:00, a NEW shift begins with a fresh 11/14. The 3h D + 5h OFF after 16:00 are part of Shift 2, ending at 06:00 next day.",
      },
      {
        ...
        description: "Driver attempts a 3+7 pairing but it fails two ways: (1) the SB is too short (<7h), and (2) the 7h rest is Off Duty, not Sleeper Berth. No valid split → 14h wall-clock closes at 14:00. Driving at 21:00 is 7h past shift end → 14-hr violation.",
      },
    ],
  },
  {
    ...
    extraExamples: [
      ...
      },
      {
        ...
        description: "A 10-hour SB straight through midnight (19:00 Day 1 → 05:00 Day 2) is a complete 10-hr reset on its own — NOT a split. CVSA 10-Hour Continuous Break rule applies: shift ENDS at the BEGINNING of the 10-hr reset (19:00 Day 1). After the 10h SB completes at 05:00 Day 2, a new shift begins with a fresh 11/14 — the 1h OD + 5h D on Day 2 are the next shift, ending at 11:00 when the 13h OFF reset begins.",
      },
    ],
  },
];
