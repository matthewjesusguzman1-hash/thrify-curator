    ...
    try {
      ...
      if (mode === "combined") {
        ...
        if (files.length === 1) {
          ...
          try {
            // iOS writes both `text` AND `title` out as separate .txt files
            // alongside multi-file shares when "Save to Files" is picked.
            // Pass only the files to keep the output clean.
