...
export default function HoursOfServicePage() {
  ...
  const oosSim = useMemo(() => {
    ...
    steps.push({
      ...
      description: `Rest remainder of today (${fmt(hLeft)} hr). At midnight, oldest day ${DAY_NAMES[oldestDate0.getDay()]} ${oldestDate0.getMonth() + 1}/${oldestDate0.getDate()} (${fmt(gained1)} hr) is removed from the calculation.`,
      gained: gained1,
      runningTotal,
    ...
    while (runningTotal > limit && cumulativeOOS < 34 && daysDropped < dayTotals.length) {
      ...
      steps.push({
        ...
        oosHours: cumulativeOOS,
        description: `+24 hr rest. Next oldest day ${DAY_NAMES[dropDate.getDay()]} ${dropDate.getMonth() + 1}/${dropDate.getDate()} (${fmt(gainedN)} hr) is removed from the calculation.`,
        gained: gainedN,
        runningTotal,
        available: Math.max(0, limit - runningTotal),
        passes: runningTotal <= limit,
                  ...
                  <li key={s.stepNum} className="flex gap-2" data-testid={`hos-oos-step-${s.stepNum}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 text-[11px] flex flex-wrap gap-x-2 gap-y-0.5">
                        ...
                        <span><span className="text-[#64748B]">Removed from calculation:</span> <strong className="text-[#475569]">−{fmt(s.gained)} hr</strong></span>
                        <span className="text-[#CBD5E1]">·</span>
