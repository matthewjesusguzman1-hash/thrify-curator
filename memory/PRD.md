...
## Recent Changes
- **HOS UX Overhaul for First-Time Users (2026-04-20)**: (a) Big verdict banner at top — "WITHIN LIMIT · N hr available" or "OUT OF SERVICE" with total shown prominently, (b) replaced text "hours remaining in today's log" input with a time-of-stop picker ("When did you stop them?" → auto-computes hours until midnight), (c) Recovery Logic hidden behind a "Show the math" toggle so new users see only the recommendation, (d) ? help popovers on rule type, daily hours, and stop time explaining each in plain language, (e) dismissible 3-step walkthrough card at top (localStorage-persisted), (f) simplified rows — Drive/On-Duty hidden by default behind "Show split" checkbox, only Total column visible. Added numbered ① ② ③ step headers.
- **HOS Save & Export (2026-04-20)**: Wired the WLL/TieDown pattern onto the HOS calculator. Added a sticky "Preview & Export" + "Save to Inspection" floating action bar. Preview opens a dedicated **Hours of Service Recap** PDF (header + totals card + daily log table + recovery logic steps + CFR footer) via the shared `PDFPreview` + html2canvas/jsPDF pipeline. Save modal matches TieDown — New Inspection inline create + existing list. Backend: added `POST /api/inspections/{id}/hos` + `DELETE /api/inspections/{id}/hos/{assessment_id}` endpoints persisting a full snapshot (rule type, limit, 8/7 daily rows, totals, OOS state, hours-left input, recovery steps, 34-hr restart recommendation) into `hos_assessments[]` on the inspection doc.
