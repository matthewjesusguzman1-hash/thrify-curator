...
async def email_shift_report(email_req: EmailReportRequest, admin: dict = Depends(get_admin_user)):
    """Email shift report to specified recipient"""
    if not RESEND_API_KEY or RESEND_API_KEY == 're_123_placeholder':
        ...
        <div class="summary">
            ...
            <div class="summary-grid">
                <div class="summary-item">
                    <div class="summary-value">{summary.get('total_hours', 0)}</div>
                    <div class="summary-label">Total Hours</div>
                </div>
                <div class="summary-item">
                    <div class="summary-value">{summary.get('total_shifts', 0)}</div>
                    <div class="summary-label">Total Shifts</div>
                </div>
                <div class="summary-item">
                    <div class="summary-value">{summary.get('employee_count', 0)}</div>
    ...
    for emp in by_employee:
        html_content += f"""
                <tr>
                    <td class="employee-name">{emp.get('name', 'Unknown')}</td>
...
async def generate_payroll_report(request: PayrollReportRequest, admin: dict = Depends(get_admin_user)):
    ...
    # Default hourly rate from settings (used when employee has no individual rate)
...
@api_router.post("/admin/payroll/report/email")
async def email_payroll_report(email_req: EmailPayrollRequest, admin: dict = Depends(get_admin_user)):
    ...
    <head>
        <style>
            ...
            .summary-card {{ background: #f9f6f7; padding: 15px; border-radius: 8px; text-align: center; }}
            .summary-value {{ font-size: 24px; font-weight: bold; color: #333; }}
            .summary-label {{ font-size: 12px; color: #888; }}
            table {{ width: 100%; border-collapse: collapse; margin-top: 20px; }}
    ...
    <body>
        ...
        <div class="summary-grid">
            <div class="summary-card">
                <div class="summary-value">{summary.get('total_employees', 0)}</div>
                <div class="summary-label">Employees</div>
            </div>
            <div class="summary-card">
                <div class="summary-value">{summary.get('total_hours', 0):.2f}</div>
                <div class="summary-label">Total Hours</div>
            </div>
            <div class="summary-card">
                <div class="summary-value">{summary.get('total_shifts', 0)}</div>
            ...
            <div class="summary-card">
                <div class="summary-value">${summary.get('total_wages', 0):.2f}</div>
...
@api_router.get("/admin/forms/job-applications")
...
@api_router.get("/admin/forms/consignment-inquiries")
...
@api_router.get("/admin/forms/consignment-agreements")
...
@api_router.put("/admin/forms/job-applications/{submission_id}/status")
...
@api_router.put("/admin/forms/consignment-inquiries/{submission_id}/status")
...
@api_router.put("/admin/forms/consignment-agreements/{submission_id}/status")
...
@api_router.delete("/admin/forms/job-applications/{submission_id}")
...
@api_router.delete("/admin/forms/consignment-inquiries/{submission_id}")
...
@api_router.delete("/admin/forms/consignment-agreements/{submission_id}")
...
@api_router.get("/admin/forms/summary")
