...
from models import TimeEntry, ClockInOut
from services.notifications import create_admin_notification, send_clock_notification_email, get_employee_hours_summary
    ...
    if action.action == "in":
        ...
        if today_entry:
            ...
            asyncio.create_task(send_clock_notification_email(
                action="in", employee_name=user["name"], timestamp=now_iso
            ))
        ...
        session_hours = round((now - clock_in_time).total_seconds() / 3600, 2)
        ...
        asyncio.create_task(send_clock_notification_email(
            action="out", employee_name=user["name"], timestamp=now_iso, hours_summary=hours_summary
        ))
