...
async def send_application_rejection_email(
    to_email: str, 
    applicant_name: str,
    keep_on_file_url: str
) -> dict:
    """Send a soft rejection email to a job applicant with option to keep application on file"""
    ...
    <p style="color: #333; line-height: 1.6;">
        Thank you so much for taking the time to apply to <strong>Thrifty Curator</strong>. 
    ...
    <div style="background: #f8f4ff; border-left: 4px solid #8B5CF6; padding: 20px; margin: 25px 0; border-radius: 0 8px 8px 0;">
        ...
        <div style="display: flex; gap: 10px;">
            <a href="{keep_on_file_url}?response=yes" 
               style="background: #8B5CF6; color: white; text-decoration: none; padding: 10px 25px; border-radius: 6px; font-weight: 500; display: inline-block;">
                Yes, keep my application
            </a>
            <a href="{keep_on_file_url}?response=no" 
               style="background: #f3f4f6; color: #374151; text-decoration: none; padding: 10px 25px; border-radius: 6px; font-weight: 500; display: inline-block; border: 1px solid #d1d5db;">
                No thanks
            </a>
        </div>
    </div>
...
def get_rejection_email_preview(applicant_name: str) -> dict:
    """Get a preview of the rejection email content (for admin preview before sending)"""
    
    first_name = applicant_name.split()[0] if applicant_name else "there"
    
    return {
        "subject": "Thrifty Curator - Thank You for Applying",
