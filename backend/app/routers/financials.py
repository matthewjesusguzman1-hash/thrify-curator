...
async def get_tax_documents(year: int, document_type: Optional[str] = None, include_1099s: bool = False):
    """Get tax documents for a year (excludes 1099s by default for Step 4 view)"""
    query = {"year": year}
    if document_type:
        query["document_type"] = document_type
    elif not include_1099s:
        # Exclude 1099_nec entries by default (they're shown in Issued 1099s page)
        query["type"] = {"$ne": "1099_nec"}
...
async def download_tax_summary(year: int, format: str = "pdf"):
    ...
    if format == "csv":
        ...
        # Add contractor payments (1099s issued)
        issued_1099s = await db.issued_1099s.find({"year": year}, {"_id": 0}).to_list(length=None)
        if issued_1099s:
            y_pos -= 0.6*inch
            c.setFont("Helvetica-Bold", 12)
            c.drawString(1*inch, y_pos, "1099-NEC PAYMENTS TO CONTRACTORS")
            y_pos -= 0.25*inch
            c.setLineWidth(0.5)
            c.line(1*inch, y_pos + 0.1*inch, width - 1*inch, y_pos + 0.1*inch)
            
            c.setFont("Helvetica", 10)
            total_contractor_payments = 0
            for contractor in issued_1099s:
                y_pos -= 0.2*inch
                c.drawString(1.2*inch, y_pos, contractor.get('contractor_name', 'Unknown'))
                c.drawRightString(width - 1*inch, y_pos, f"${contractor.get('amount_paid', 0):,.2f}")
                total_contractor_payments += contractor.get('amount_paid', 0)
            
            y_pos -= 0.25*inch
            c.setFont("Helvetica-Bold", 11)
            c.drawString(1*inch, y_pos, "Total Contractor Payments:")
            c.drawRightString(width - 1*inch, y_pos, f"${total_contractor_payments:,.2f}")
...
@router.get("/my-1099s/{year}")
async def get_my_1099s(year: int, user_id: str = None):
    """Get 1099s issued to a specific user/employee for their portal view"""
    if not user_id:
        raise HTTPException(status_code=400, detail="user_id is required")
    
    # Get tax documents for this user
    docs = await db.tax_documents.find(
        {"user_id": user_id, "year": year, "type": "1099_nec"},
        {"_id": 0}
