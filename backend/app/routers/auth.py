...
from app.config import OWNER_CODES
...
from app.services.security import (
    ...
    hash_user_password, verify_user_password
)
...
def verify_employee_password(password: str, stored_hash: str) -> bool:
    ...
    ok, _ = verify_user_password(password, stored_hash)
    return ok
...
async def login(credentials: UserLogin):
    ...
    lock_id = f"login:{email_lower}"
    await check_lockout(lock_id)
    
    user = await db.users.find_one({"email": {"$regex": f"^{re.escape(email_lower)}$", "$options": "i"}}, {"_id": 0})
    if not user:
    ...
    if user["role"] == "admin":
        ...
        # First check owner codes (configured via environment)
        ...
        if user_admin_code and secrets.compare_digest(credentials.admin_code, user_admin_code):
            await clear_attempts(lock_id)
    ...
    if has_password:
        ...
        ok, needs_rehash = verify_user_password(credentials.password, employee_password_hash)
        if not ok:
            await record_failed_attempt(lock_id)
            raise HTTPException(status_code=401, detail="Invalid password")
        
        # Transparent migration: rehash legacy sha256 to bcrypt on successful login
        if needs_rehash:
            await db.users.update_one(
                {"id": user["id"]},
                {"$set": {"password_hash": hash_user_password(credentials.password)}}
            )
        await clear_attempts(lock_id)
...
async def lock_account(request: AccountLockRequest, admin: dict = Depends(get_current_user)):
    ...
    if request.user_type == "employee":
        # Find the employee
        user = await db.users.find_one({"email": {"$regex": f"^{re.escape(email_lower)}$", "$options": "i"}})
        if not user:
        ...
        # Find the consignor
        agreement = await db.consignment_agreements.find_one({"email": {"$regex": f"^{re.escape(email_lower)}$", "$options": "i"}})
        if not agreement:
